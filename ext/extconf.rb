...
def check_libmemcached
  ...
  Dir.chdir(HERE) do
    Dir.chdir(BUNDLE_PATH) do
      ...
      run("env CFLAGS='-fPIC #{LIBM_CFLAGS}' LDFLAGS='-fPIC #{LIBM_LDFLAGS}' ./configure --prefix=#{HERE} --libdir=#{HERE}/lib --without-memcached --disable-shared --disable-utils --disable-dependency-tracking #{$CC} #{$EXTRA_CONF} #{ENV["MEMCACHE_EXTRA_CONF"]} 2>&1", "Configuring libmemcached.")
    end
